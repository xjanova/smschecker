package com.thaiprompt.smschecker.ui.theme

import androidx.compose.runtime.compositionLocalOf

data class AppStrings(
    // === Navigation ===
    val navDashboard: String,
    val navOrders: String,
    val navTransactions: String,
    val navSettings: String,

    // === Dashboard ===
    val todayIncome: String,
    val todayExpense: String,
    val syncStatus: String,
    val syncPending: String,       // "{count} รอดำเนินการ"
    val syncComplete: String,
    val syncButton: String,
    val serverStatus: String,
    val serverOffline: String,
    val serverWaiting: String,
    val serverJustNow: String,
    val serverConnected: String,
    val running: String,
    val stopped: String,
    val refresh: String,
    val recentTransactions: String,
    val noTransactions: String,
    val smsAutoDisplay: String,
    val synced: String,
    val pending: String,
    val orderApproval: String,
    val revenueChartTitle: String,
    val autoApproved: String,
    val manuallyApproved: String,
    val pendingReview: String,
    val rejected: String,
    val last7Days: String,
    val bahtSymbol: String,
    val netBalance: String,
    val todayCount: String,
    val totalMessages: String,
    val syncRate: String,
    val quickActions: String,
    val systemOverview: String,
    val activeDevices: String,
    val connectedServers: String,
    val offlineQueueLabel: String,
    val successRate: String,

    // === Revenue Detail (chart drilldown) ===
    val revenueDetailTitle: String,
    val revenueDetailSubtitle: String,
    val tapChartHint: String,
    val periodWeek: String,
    val periodMonth: String,
    val periodYear: String,
    val periodCustom: String,
    val billChartTitle: String,
    val billChartSubtitle: String,
    val bankChartTitle: String,
    val bankChartSubtitle: String,
    val billRefund: String,
    val billIncomeTotal: String,
    val bankCreditTotal: String,
    val bankDebitTotal: String,
    val customStartDate: String,
    val customEndDate: String,
    val applyRange: String,
    val noBillsInRange: String,
    val noBankActivity: String,
    val totalIncomeLabel: String,
    val totalExpenseLabel: String,
    val avgPerBucket: String,
    val maxIncome: String,
    val maxExpense: String,
    val back: String,

    // === Orders ===
    val ordersTitle: String,
    val approvalManagement: String,
    val approveAll: String,
    val filterAll: String,
    val filterPendingReview: String,
    val filterAutoApproved: String,
    val filterApproved: String,
    val filterRejected: String,
    val filterDate: String,
    val filtered: String,
    val clearFilter: String,
    val totalLabel: String,
    val pendingCheck: String,
    val offlineQueue: String,
    val noOrders: String,
    val matchedOrdersWillShow: String,
    val searchPlaceholder: String,
    val loadingMore: String,
    val unknown: String,
    val ambiguousMatch: String,
    val queuedLabel: String,
    val rejectButton: String,
    val approveButton: String,
    val statusAutoApproved: String,
    val statusApproved: String,
    val statusPendingReview: String,
    val statusRejected: String,
    val statusExpired: String,
    val statusCancelled: String,
    val statusDeleted: String,

    // === Transactions ===
    val transactionsTitle: String,
    val paymentHistory: String,
    val filterCredit: String,
    val filterDebit: String,
    val syncedCount: String,
    val pendingCount: String,
    val noTransactionsFound: String,

    // === Settings ===
    val settingsTitle: String,
    val configAndSettings: String,
    val deviceInfo: String,
    val monitorSms: String,
    val monitoringActive: String,
    val monitoringPaused: String,
    val approvalModeTitle: String,
    val queueCount: String,        // "{count} รอคิว"
    val serverConnections: String,
    val scanQr: String,
    val addManual: String,
    val noServers: String,
    val addServerToSync: String,
    val scanQrToAddServer: String,
    val supportedBanks: String,
    val primaryBadge: String,
    val lastSyncLabel: String,
    val neverSynced: String,
    val successStatus: String,
    val failedStatus: String,
    val removeButton: String,
    val removeServerTitle: String,
    val removeServerMessage: String,
    val cancelButton: String,
    val addServerTitle: String,
    val addServerSubtitle: String,
    val serverNameLabel: String,
    val serverNamePlaceholder: String,
    val serverUrlLabel: String,
    val secretKeyLabel: String,
    val setAsDefault: String,
    val saveButton: String,
    val versionInfo: String,

    // === Theme ===
    val themeMode: String,
    val themeDark: String,
    val themeLight: String,
    val themeSystem: String,

    // === Language ===
    val languageTitle: String,
    val languageThai: String,
    val languageEnglish: String,
    val languageSystem: String,

    // === QR Scanner ===
    val qrScannerTitle: String,
    val qrScannerSubtitle: String,
    val cameraPermissionRequired: String,
    val cameraPermissionDenied: String,
    val cameraFailed: String,
    val pointCameraAtQr: String,
    val howToGetQrCode: String,
    val qrInstructions: String,
    val allowCamera: String,
    val qrDefaultServerName: String,

    // === SMS History ===
    val smsMatcherTitle: String,
    val smsMatcherSubtitle: String,
    val customBankMapping: String,
    val customBankMappingDesc: String,
    val totalDetected: String,
    val totalSynced: String,
    val allTypes: String,
    val creditOnly: String,
    val debitOnly: String,
    val noTransactionsYet: String,
    val syncedLabel: String,
    val pendingLabel: String,
    // Legacy fields (kept for compatibility)
    val addRule: String,
    val noRules: String,
    val noRulesDescription: String,
    val recentSmsTitle: String,
    val assignBank: String,
    val senderAddressLabel: String,
    val sampleMessageLabel: String,
    val selectBankLabel: String,
    val activeRules: String,
    val deleteRule: String,
    val deleteRuleConfirm: String,
    val tapToAssign: String,

    // === Bank Names ===
    val bankKbank: String,
    val bankScb: String,
    val bankKtb: String,
    val bankBbl: String,
    val bankGsb: String,
    val bankBay: String,
    val bankTtb: String,
    val bankPromptPay: String,
    val bankCimb: String,
    val bankKkp: String,
    val bankLh: String,
    val bankTisco: String,
    val bankUob: String,
    val bankIcbc: String,
    val bankBaac: String,

    // === SMS Auto Scan ===
    val autoScanTitle: String,
    val autoScanSubtitle: String,
    val detectedBankSms: String,
    val unknownSender: String,
    val allOtherSms: String,
    val tapToAssignBank: String,
    val autoDetected: String,
    val customRule: String,
    val matchedWithOrder: String,
    val noMatchingOrder: String,
    val scanInbox: String,
    val scanInboxHint: String,
    val scanning: String,
    val scanComplete: String,
    val foundMessages: String,  // "{count} messages found"

    // === TTS ===
    val ttsTitle: String,
    val ttsDescription: String,
    val ttsEnabled: String,
    val ttsDisabled: String,
    val ttsCreditAnnounce: String, // "เงินเข้า {bank} จำนวน {amount} บาท"
    val ttsDebitAnnounce: String,  // "เงินออก {bank} จำนวน {amount} บาท"
    val ttsOrderMatch: String,     // "ตรงกับออเดอร์ {order}"
    val ttsLanguageLabel: String,
    val ttsLangAuto: String,
    val ttsLangThai: String,
    val ttsLangEnglish: String,
    val ttsSpeakContentLabel: String,
    val ttsSpeakTypeLabel: String,
    val ttsSpeakBankLabel: String,
    val ttsSpeakAmountLabel: String,
    val ttsSpeakOrderLabel: String,
    val ttsSpeakCustomerLabel: String,
    val ttsSpeakProductLabel: String,
    val ttsPreviewButton: String,

    // === Notification Listening ===
    val notificationListeningTitle: String,
    val notificationListeningActive: String,
    val notificationListeningPaused: String,
    val notificationAccessRequired: String,
    val notificationBadge: String,

    // === SMS Permission ===
    val smsPermissionRequired: String,

    // === SMS History Edit ===
    val editTransaction: String,
    val editBank: String,
    val editType: String,
    val editAmount: String,
    val editSave: String,
    val editCancel: String,
    val editRawMessage: String,
    val editIncorrectData: String,
    val navSmsHistory: String,

    // === Common ===
    val backButton: String,
) {
    // === Millennium 3D redesign strings (Thai for both languages; refine EN later) ===
    // CLASS-BODY properties, NOT constructor parameters — adding these ~52 fields to
    // the (already ~233-param) constructor pushed the synthetic default-args
    // constructor past the dex 255-argument-register limit. That compiles, but the
    // ART verifier rejects AppStringsKt.<clinit> at runtime (VerifyError) and the
    // app crashes on every launch. Keep them out of the constructor.
    val aeroIncomeToday: String = "เงินเข้าวันนี้"
    val aeroConnected: String = "เชื่อมต่อ"
    // ปุ่ม Force อนุมัติ (ปุ่มหลัก ใช้บ่อยกว่าอนุมัติปกติ)
    val forceApproveButton: String = "Force อนุมัติ"
    val forceApproveHint: String = "อนุมัติทันทีโดยไม่ผ่านการจับคู่ SMS"
    // badge วิธีอนุมัติบนการ์ดบิล (ApprovalMethod)
    val approvedViaSms: String = "อนุมัติผ่าน SMS"
    val approvedViaSlip: String = "อนุมัติผ่านสลิป"
    val approvedViaAdmin: String = "แอดมินอนุมัติ"
    // 🧾 (2026-07-27) สลิป SlipOK ติดบิล — ทัมบ์เนลบนการ์ด + จอดูรูปเต็ม
    val slipViewerTitle: String = "สลิปที่ใช้อนุมัติ"
    val slipTapHint: String = "แตะดูสลิป"
    val slipTransRefLabel: String = "เลขอ้างอิง"
    val slipSenderLabel: String = "ผู้โอน"
    val slipReceiverLabel: String = "เข้าบัญชี"
    val slipCheckedAtLabel: String = "เวลาตรวจสลิป"
    val slipAmountLabel: String = "ยอดบนสลิป"
    val slipLoadFailed: String = "โหลดรูปสลิปไม่ได้ — อาจถูกลบตามรอบเก็บ 30 วัน หรือเน็ตมีปัญหา"
    val slipCloseButton: String = "ปิด"
    // 🚫 (2026-07-27) ยกเลิกการอนุมัติบิลที่อนุมัติผิด
    val voidApprovalButton: String = "ยกเลิกการอนุมัติ"
    val voidApprovalTitle: String = "ยกเลิกการอนุมัติบิลนี้?"
    val voidApprovalBody: String = "บิลจะกลับเป็น \"ยังไม่ได้ชำระ\" — ระบบจะคืนยอดทศนิยมเฉพาะ ปลด SMS ที่ผูกไว้ และดึงค่าคอมมิชชั่นที่จ่ายไปแล้วคืน"
    val voidApprovalHint: String = "ใช้เมื่อสลิปปลอม/สลิปเก่า หรืออนุมัติผิดบิล — ยังกด Force อนุมัติใหม่ได้ถ้าเงินเข้าจริงภายหลัง"
    val voidApprovalConfirm: String = "ยืนยันยกเลิก"
    val voidConsumedTitle: String = "ลูกค้าใช้บริการไปแล้ว"
    val voidConsumedBody: String = "บิลนี้ลูกค้าเปิดไพ่หรือได้รับคำทำนายไปแล้ว ถ้ายกเลิกการอนุมัติจะถือว่าบิลไม่ได้ชำระทั้งที่ให้บริการไปแล้ว"
    val voidConsumedConfirm: String = "ยืนยันยกเลิกอยู่ดี"
    // TTS voice health (เคส Samsung ไม่มีเสียงไทย)
    val ttsVoiceWarningTitle: String = "เสียงพูดอาจไม่ทำงานบนเครื่องนี้"
    val ttsGoogleMissingDesc: String = "ไม่พบ Google Text-to-Speech ในเครื่อง (Samsung มักไม่มีเสียงภาษาไทย) — ติดตั้งฟรีจาก Play Store แล้วแอพจะใช้เสียงไทยได้ทันที"
    val ttsThaiVoiceMissingDesc: String = "มีโปรแกรมเสียงแล้ว แต่ยังไม่ได้ดาวน์โหลดชุดเสียงภาษาไทย — แตะปุ่มด้านล่างเพื่อดาวน์โหลด (ฟรี ใช้ครั้งเดียว)"
    val ttsInstallGoogleButton: String = "ติดตั้ง Google TTS (ฟรี)"
    val ttsDownloadVoiceButton: String = "ดาวน์โหลดชุดเสียงไทย"
    // 🔁 (2026-08-08) ปุ่มตรวจซ้ำ — ระบบเช็ค+ซ่อมเองทุก 15 นาทีอยู่แล้ว ปุ่มนี้คือทางลัดให้กดเดี๋ยวนั้น
    val ttsRecheckButton: String = "ตรวจสอบเสียงอีกครั้ง"
    val ttsRecheckingLabel: String = "กำลังตรวจสอบเสียง…"
    val ttsRecheckStillMissing: String = "ตรวจซ้ำแล้วยังไม่พบชุดเสียงไทย — ลองกดปุ่มดาวน์โหลดด้านบน แล้วเลือก \"ไทย\" ในหน้าที่เปิดขึ้นมา"
    val aeroPopularBanks: String = "ธนาคารยอดนิยม"
    val aeroViewAll: String = "ดูทั้งหมด"
    val aeroLive: String = "สด"
    val aeroMatched: String = "จับคู่"
    val aeroCreditCount: String = "เครดิต"
    val aeroDebitCount: String = "เดบิต"
    val aeroTotalCount: String = "ยอดรวม"
    val aeroAmountToTransfer: String = "ยอดที่ต้องโอน"
    val aeroUniqueDecimal: String = "ยอดที่ต้องโอน (เลขทศนิยมเฉพาะ)"
    val aeroWaitingMatch: String = "รอจับคู่"
    val aeroMatchedDone: String = "จับคู่แล้ว"
    val aeroSelectedPrefix: String = "เลือก"
    val aeroItemsSuffix: String = "รายการ"
    val aeroStatsTitle: String = "สถิติ"
    val aeroOverview7Days: String = "ภาพรวม 7 วันล่าสุด"
    val aeroAmountByBank: String = "ยอดตามธนาคาร"
    val aeroThisWeek: String = "สัปดาห์นี้"
    val aeroMatchRate: String = "จับคู่สำเร็จ"
    val aeroSuccessSuffix: String = "สำเร็จ"
    val aeroPendingSuffix: String = "รอ"
    val aeroSegDay: String = "วัน"
    val aeroSegWeek: String = "สัปดาห์"
    val aeroSegMonth: String = "เดือน"
    val aeroIncomeThisWeek: String = "เงินเข้ารวมสัปดาห์นี้"
    val aeroAvgPerDay: String = "เฉลี่ย / วัน"
    val aeroPeakDay: String = "สูงสุด"
    val aeroTxnCount: String = "ธุรกรรม"
    val aeroAvgMatchTime: String = "เวลาจับคู่เฉลี่ย"
    val aeroEncryptedConnection: String = "การเชื่อมต่อที่เข้ารหัส"
    val aeroStartScanQr: String = "เริ่มสแกน QR Code"
    val aeroEnterManually: String = "ใส่รหัสด้วยตนเอง"
    val aeroConnectDevice: String = "เชื่อมต่ออุปกรณ์"
    val aeroConnectDeviceSub: String = "สแกน QR จากแผงควบคุมเพื่อตั้งค่าอัตโนมัติ"
    val aeroServerField: String = "เซิร์ฟเวอร์"
    val aeroEncryptionField: String = "การเข้ารหัส"
    val aeroSignatureField: String = "ลายเซ็น"
    val aeroAccountPrefix: String = "บัญชี"
    val aeroIncomingLabel: String = "เงินเข้า"
    val aeroOutgoingLabel: String = "เงินออก"
    val aeroToday: String = "วันนี้"
    val aeroYesterday: String = "เมื่อวาน"
    val aeroTransactionsTitle: String = "รายการเงินเข้า"
    val aeroSmsTitle: String = "ข้อความ SMS"
    val aeroLatestMessages: String = "ข้อความล่าสุด"
    val aeroOrdersTitle: String = "จัดการออเดอร์"
    val aeroPendingApprovalSuffix: String = "รายการรอการอนุมัติ"
    val aeroReceivedAt: String = "ได้รับเมื่อ"
    val aeroExpiresIn: String = "หมดอายุใน"
    val aeroSegPending: String = "รอ"
    val aeroSegApproved: String = "อนุมัติ"
    val aeroSegAll: String = "ทั้งหมด"
    val aeroDashboardTitle: String = "แดชบอร์ด"
}

val ThaiStrings = AppStrings(
    // Navigation
    navDashboard = "หน้าหลัก",
    navOrders = "ออเดอร์",
    navTransactions = "รายการ",
    navSettings = "ตั้งค่า",

    // Dashboard
    todayIncome = "รายรับวันนี้",
    todayExpense = "รายจ่ายวันนี้",
    syncStatus = "สถานะการซิงค์",
    syncPending = "รอดำเนินการ",
    syncComplete = "ซิงค์ครบแล้ว",
    syncButton = "ซิงค์",
    serverStatus = "สถานะเซิร์ฟเวอร์",
    serverOffline = "ออฟไลน์",
    serverWaiting = "รอ sync...",
    serverJustNow = "เมื่อสักครู่",
    serverConnected = "เชื่อมต่อแล้ว",
    running = "ทำงาน",
    stopped = "หยุด",
    refresh = "รีเฟรช",
    recentTransactions = "รายการล่าสุด",
    noTransactions = "ยังไม่มีรายการ",
    smsAutoDisplay = "SMS ธนาคารจะแสดงที่นี่อัตโนมัติ",
    synced = "ซิงค์แล้ว",
    pending = "รอดำเนินการ",
    orderApproval = "อนุมัติออเดอร์",
    revenueChartTitle = "รายได้",
    revenueDetailTitle = "รายละเอียดรายได้",
    revenueDetailSubtitle = "เงินเข้า / เงินออก ย้อนหลัง",
    tapChartHint = "👆 แตะที่กราฟเพื่อดูรายละเอียดแต่ละจุด",
    periodWeek = "7 วัน",
    periodMonth = "30 วัน",
    periodYear = "1 ปี",
    periodCustom = "กำหนดเอง",
    billChartTitle = "รายได้บิล",
    billChartSubtitle = "แยกตามเซิร์ฟเวอร์",
    bankChartTitle = "SMS ธนาคาร",
    bankChartSubtitle = "เงินเข้า / เงินออก ดิบจาก SMS",
    billRefund = "บิลคืนเงิน",
    billIncomeTotal = "บิลรวม",
    bankCreditTotal = "เงินเข้ารวม",
    bankDebitTotal = "เงินออกรวม",
    customStartDate = "วันเริ่ม",
    customEndDate = "วันสิ้นสุด",
    applyRange = "เลือกช่วง",
    noBillsInRange = "ไม่มีบิลที่อนุมัติในช่วงนี้",
    noBankActivity = "ไม่มีการเคลื่อนไหวจากธนาคาร",
    totalIncomeLabel = "เงินเข้ารวม",
    totalExpenseLabel = "เงินออกรวม",
    avgPerBucket = "เฉลี่ย",
    maxIncome = "เงินเข้าสูงสุด",
    maxExpense = "เงินออกสูงสุด",
    back = "กลับ",
    autoApproved = "อนุมัติอัตโนมัติ",
    manuallyApproved = "อนุมัติด้วยตนเอง",
    pendingReview = "รอตรวจสอบ",
    rejected = "ปฏิเสธ",
    last7Days = "7 วันที่ผ่านมา",
    bahtSymbol = "฿",
    netBalance = "ยอดสุทธิวันนี้",
    todayCount = "รายการวันนี้",
    totalMessages = "ข้อความทั้งหมด",
    syncRate = "อัตราซิงค์",
    quickActions = "ดำเนินการด่วน",
    systemOverview = "ภาพรวมระบบ",
    activeDevices = "อุปกรณ์",
    connectedServers = "เซิร์ฟเวอร์",
    offlineQueueLabel = "คิวออฟไลน์",
    successRate = "สำเร็จ",

    // Orders
    ordersTitle = "ออเดอร์",
    approvalManagement = "จัดการการอนุมัติ",
    approveAll = "อนุมัติทั้งหมด",
    filterAll = "ทั้งหมด",
    filterPendingReview = "รอตรวจสอบ",
    filterAutoApproved = "อัตโนมัติ",
    filterApproved = "อนุมัติแล้ว",
    filterRejected = "ปฏิเสธ",
    filterDate = "วันที่",
    filtered = "กรองแล้ว",
    clearFilter = "ล้าง",
    totalLabel = "ทั้งหมด",
    pendingCheck = "รอตรวจ",
    offlineQueue = "ออฟไลน์",
    noOrders = "ยังไม่มีออเดอร์",
    matchedOrdersWillShow = "รายการชำระเงินที่ตรงกันจะแสดงที่นี่",
    searchPlaceholder = "ค้นหาเลขบิล, ชื่อสินค้า, ลูกค้า...",
    loadingMore = "กำลังโหลด...",
    unknown = "ไม่ทราบ",
    ambiguousMatch = "การจับคู่ไม่ชัดเจน",
    queuedLabel = "รอคิว",
    rejectButton = "ปฏิเสธ",
    approveButton = "อนุมัติ",
    statusAutoApproved = "อัตโนมัติ",
    statusApproved = "อนุมัติแล้ว",
    statusPendingReview = "รอตรวจ",
    statusRejected = "ปฏิเสธ",
    statusExpired = "หมดอายุ",
    statusCancelled = "ยกเลิก",
    statusDeleted = "ลบแล้ว",

    // Transactions
    transactionsTitle = "รายการ",
    paymentHistory = "ประวัติการชำระเงิน",
    filterCredit = "รายรับ",
    filterDebit = "รายจ่าย",
    syncedCount = "ซิงค์แล้ว",
    pendingCount = "รอดำเนินการ",
    noTransactionsFound = "ไม่พบรายการ",

    // Settings
    settingsTitle = "ตั้งค่า",
    configAndSettings = "การกำหนดค่าและการตั้งค่า",
    deviceInfo = "ข้อมูลอุปกรณ์",
    monitorSms = "ติดตาม SMS",
    monitoringActive = "ทำงานอยู่ - รอรับ SMS ธนาคาร",
    monitoringPaused = "หยุดชั่วคราว",
    approvalModeTitle = "โหมดอนุมัติ",
    queueCount = "รอคิว",
    serverConnections = "เชื่อมต่อเซิร์ฟเวอร์",
    scanQr = "สแกน QR",
    addManual = "เพิ่มเอง",
    noServers = "ยังไม่มีเซิร์ฟเวอร์",
    addServerToSync = "เพิ่มเซิร์ฟเวอร์เพื่อซิงค์ข้อมูลการชำระเงิน",
    scanQrToAddServer = "สแกน QR Code เพิ่มเซิร์ฟเวอร์",
    supportedBanks = "ธนาคารที่รองรับ",
    primaryBadge = "หลัก",
    lastSyncLabel = "ซิงค์ล่าสุด",
    neverSynced = "ยังไม่เคยซิงค์",
    successStatus = "สำเร็จ",
    failedStatus = "ล้มเหลว",
    removeButton = "ลบ",
    removeServerTitle = "ลบเซิร์ฟเวอร์",
    removeServerMessage = "ต้องการลบออกจากการเชื่อมต่อหรือไม่? ข้อมูลบนเซิร์ฟเวอร์จะไม่ถูกลบ",
    cancelButton = "ยกเลิก",
    addServerTitle = "เพิ่มเซิร์ฟเวอร์",
    addServerSubtitle = "เชื่อมต่อกับ Laravel เพื่อซิงค์ข้อมูลการชำระเงิน",
    serverNameLabel = "ชื่อเซิร์ฟเวอร์",
    serverNamePlaceholder = "เช่น Thaiprompt Main",
    serverUrlLabel = "URL เซิร์ฟเวอร์",
    secretKeyLabel = "Secret Key (สำหรับเข้ารหัส)",
    setAsDefault = "ตั้งเป็นเซิร์ฟเวอร์หลัก",
    saveButton = "บันทึก",
    versionInfo = "SMS Payment Checker v${com.thaiprompt.smschecker.BuildConfig.VERSION_NAME}\n\u00A9 2025 Xman Studio",

    // Theme
    themeMode = "ธีม",
    themeDark = "มืด",
    themeLight = "สว่าง",
    themeSystem = "ตามระบบ",

    // Language
    languageTitle = "ภาษา",
    languageThai = "ไทย",
    languageEnglish = "English",
    languageSystem = "ตามระบบ",

    // QR Scanner
    qrScannerTitle = "สแกน QR Code",
    qrScannerSubtitle = "เพิ่มเซิร์ฟเวอร์จาก QR Code",
    cameraPermissionRequired = "ต้องอนุญาตใช้กล้อง",
    cameraPermissionDenied = "ต้องอนุญาตใช้กล้องเพื่อสแกน QR Code",
    cameraFailed = "เปิดกล้องไม่สำเร็จ",
    pointCameraAtQr = "เล็งกล้องไปที่ QR Code ของเซิร์ฟเวอร์",
    howToGetQrCode = "วิธีรับ QR Code",
    qrInstructions = "1. เปิดหน้า Admin ของเว็บไซต์\n2. ไปที่ตั้งค่า SMS Checker\n3. กด 'แสดง QR Code' สำหรับอุปกรณ์\n4. สแกน QR Code ที่แสดงบนหน้าจอ",
    allowCamera = "อนุญาตใช้กล้อง",
    qrDefaultServerName = "เซิร์ฟเวอร์จาก QR",

    // SMS History
    smsMatcherTitle = "ประวัติข้อความ",
    smsMatcherSubtitle = "ข้อความธนาคารที่ตรวจจับได้",
    customBankMapping = "ประวัติข้อความธนาคาร",
    customBankMappingDesc = "ดูข้อความธนาคารที่ตรวจจับได้",
    totalDetected = "ตรวจจับ",
    totalSynced = "ซิงค์แล้ว",
    allTypes = "ทั้งหมด",
    creditOnly = "เงินเข้า",
    debitOnly = "เงินออก",
    noTransactionsYet = "ยังไม่มีข้อความที่ตรวจจับได้",
    syncedLabel = "ซิงค์แล้ว",
    pendingLabel = "รอดำเนินการ",
    addRule = "เพิ่มกฎ",
    noRules = "ยังไม่มีกฎ",
    noRulesDescription = "เลือก SMS ด้านล่างเพื่อกำหนดธนาคาร",
    recentSmsTitle = "SMS ล่าสุด",
    assignBank = "กำหนดธนาคาร",
    senderAddressLabel = "ผู้ส่ง",
    sampleMessageLabel = "ตัวอย่างข้อความ",
    selectBankLabel = "เลือกธนาคาร",
    activeRules = "กฎที่ใช้งาน",
    deleteRule = "ลบกฎ",
    deleteRuleConfirm = "ต้องการลบกฎนี้หรือไม่?",
    tapToAssign = "แตะเพื่อกำหนดธนาคาร",

    // Bank Names
    bankKbank = "ธนาคารกสิกรไทย",
    bankScb = "ธนาคารไทยพาณิชย์",
    bankKtb = "ธนาคารกรุงไทย",
    bankBbl = "ธนาคารกรุงเทพ",
    bankGsb = "ธนาคารออมสิน",
    bankBay = "ธนาคารกรุงศรีอยุธยา",
    bankTtb = "ธนาคารทหารไทยธนชาต",
    bankPromptPay = "พร้อมเพย์",
    bankCimb = "ธนาคารซีไอเอ็มบีไทย",
    bankKkp = "ธนาคารเกียรตินาคินภัทร",
    bankLh = "ธนาคารแลนด์ แอนด์ เฮ้าส์",
    bankTisco = "ธนาคารทิสโก้",
    bankUob = "ธนาคารยูโอบี",
    bankIcbc = "ธนาคารไอซีบีซี (ไทย)",
    bankBaac = "ธ.ก.ส.",

    // SMS Auto Scan
    autoScanTitle = "สแกน SMS อัตโนมัติ",
    autoScanSubtitle = "ค้นหาข้อความธนาคารจากกล่องข้อความ",
    detectedBankSms = "SMS ธนาคารที่ตรวจพบ",
    unknownSender = "ผู้ส่งที่ไม่รู้จัก",
    allOtherSms = "SMS ทั้งหมดในเครื่อง",
    tapToAssignBank = "แตะเพื่อกำหนดธนาคาร",
    autoDetected = "ตรวจพบอัตโนมัติ",
    customRule = "กฎที่กำหนดเอง",
    matchedWithOrder = "ตรงกับออเดอร์",
    noMatchingOrder = "ไม่พบออเดอร์ที่ตรงกัน",
    scanInbox = "สแกนกล่องข้อความ",
    scanInboxHint = "กดปุ่มด้านบนเพื่อสแกนข้อความ SMS ในเครื่อง",
    scanning = "กำลังสแกน...",
    scanComplete = "สแกนเสร็จสิ้น",
    foundMessages = "พบข้อความ",

    // TTS
    ttsTitle = "อ่านออกเสียง",
    ttsDescription = "อ่านการแจ้งเตือนเงินเข้า-ออกด้วย AI",
    ttsEnabled = "เปิดอ่านออกเสียง",
    ttsDisabled = "ปิดอ่านออกเสียง",
    ttsCreditAnnounce = "เงินเข้า %s จำนวน %s บาท",
    ttsDebitAnnounce = "เงินออก %s จำนวน %s บาท",
    ttsOrderMatch = "ตรงกับออเดอร์ %s",
    ttsLanguageLabel = "ภาษาที่พูด",
    ttsLangAuto = "ตามแอพ",
    ttsLangThai = "ไทย",
    ttsLangEnglish = "English",
    ttsSpeakContentLabel = "เนื้อหาที่อ่าน",
    ttsSpeakTypeLabel = "ประเภท (เงินเข้า/ออก)",
    ttsSpeakBankLabel = "ชื่อธนาคาร",
    ttsSpeakAmountLabel = "จำนวนเงิน",
    ttsSpeakOrderLabel = "เลขออเดอร์",
    ttsSpeakCustomerLabel = "ชื่อเจ้าของบิล",
    ttsSpeakProductLabel = "ชื่อสินค้า",
    ttsPreviewButton = "ทดลองฟัง",

    // Notification Listening
    notificationListeningTitle = "ตรวจจับแจ้งเตือนธนาคาร",
    notificationListeningActive = "ทำงานอยู่ - รับแจ้งเตือนจากแอปธนาคาร",
    notificationListeningPaused = "ปิดอยู่",
    notificationAccessRequired = "ต้องเปิดสิทธิ์การเข้าถึงการแจ้งเตือนในตั้งค่าระบบ",
    notificationBadge = "แจ้งเตือน",

    // SMS Permission
    smsPermissionRequired = "ต้องเปิดสิทธิ์รับ SMS ในตั้งค่าแอป",

    // SMS History Edit
    editTransaction = "แก้ไขรายการ",
    editBank = "ธนาคาร",
    editType = "ประเภท",
    editAmount = "จำนวนเงิน",
    editSave = "บันทึก",
    editCancel = "ยกเลิก",
    editRawMessage = "ข้อความต้นฉบับ",
    editIncorrectData = "แก้ไขข้อมูลที่ไม่ถูกต้อง",
    navSmsHistory = "ข้อความ",

    // Common
    backButton = "กลับ",
)

val EnglishStrings = AppStrings(
    // Navigation
    navDashboard = "Dashboard",
    navOrders = "Orders",
    navTransactions = "History",
    navSettings = "Settings",

    // Dashboard
    todayIncome = "Today's Income",
    todayExpense = "Today's Expense",
    syncStatus = "Sync Status",
    syncPending = "pending",
    syncComplete = "All synced",
    syncButton = "Sync",
    serverStatus = "Server Status",
    serverOffline = "Offline",
    serverWaiting = "Waiting...",
    serverJustNow = "Just now",
    serverConnected = "Connected",
    running = "Active",
    stopped = "Stopped",
    refresh = "Refresh",
    recentTransactions = "Recent Transactions",
    noTransactions = "No transactions yet",
    smsAutoDisplay = "Bank SMS will appear here automatically",
    synced = "Synced",
    pending = "Pending",
    orderApproval = "Order Approval",
    revenueChartTitle = "Revenue",
    revenueDetailTitle = "Revenue Details",
    revenueDetailSubtitle = "Income / expense over time",
    tapChartHint = "👆 Tap the chart for per-point details",
    periodWeek = "7 days",
    periodMonth = "30 days",
    periodYear = "1 year",
    periodCustom = "Custom",
    billChartTitle = "Bill Revenue",
    billChartSubtitle = "By server",
    bankChartTitle = "Bank SMS",
    bankChartSubtitle = "Raw credit / debit from SMS",
    billRefund = "Bill Refund",
    billIncomeTotal = "Bills total",
    bankCreditTotal = "Total credit",
    bankDebitTotal = "Total debit",
    customStartDate = "Start date",
    customEndDate = "End date",
    applyRange = "Pick range",
    noBillsInRange = "No approved bills in this range",
    noBankActivity = "No bank activity",
    totalIncomeLabel = "Total Income",
    totalExpenseLabel = "Total Expense",
    avgPerBucket = "Avg",
    maxIncome = "Max Income",
    maxExpense = "Max Expense",
    back = "Back",
    autoApproved = "Auto Approved",
    manuallyApproved = "Manually Approved",
    pendingReview = "Pending Review",
    rejected = "Rejected",
    last7Days = "Last 7 Days",
    bahtSymbol = "฿",
    netBalance = "Net Balance Today",
    todayCount = "Today's Transactions",
    totalMessages = "Total Messages",
    syncRate = "Sync Rate",
    quickActions = "Quick Actions",
    systemOverview = "System Overview",
    activeDevices = "Devices",
    connectedServers = "Servers",
    offlineQueueLabel = "Offline Queue",
    successRate = "Success",

    // Orders
    ordersTitle = "Orders",
    approvalManagement = "Approval Management",
    approveAll = "Approve All",
    filterAll = "All",
    filterPendingReview = "Pending",
    filterAutoApproved = "Auto",
    filterApproved = "Approved",
    filterRejected = "Rejected",
    filterDate = "Date",
    filtered = "Filtered",
    clearFilter = "Clear",
    totalLabel = "Total",
    pendingCheck = "Pending",
    offlineQueue = "Offline",
    noOrders = "No orders yet",
    matchedOrdersWillShow = "Matched payments will appear here",
    searchPlaceholder = "Search order #, product, customer...",
    loadingMore = "Loading...",
    unknown = "Unknown",
    ambiguousMatch = "Ambiguous match",
    queuedLabel = "Queued",
    rejectButton = "Reject",
    approveButton = "Approve",
    statusAutoApproved = "Auto",
    statusApproved = "Approved",
    statusPendingReview = "Pending",
    statusRejected = "Rejected",
    statusExpired = "Expired",
    statusCancelled = "Cancelled",
    statusDeleted = "Deleted",

    // Transactions
    transactionsTitle = "Transactions",
    paymentHistory = "Payment History",
    filterCredit = "Income",
    filterDebit = "Expense",
    syncedCount = "Synced",
    pendingCount = "Pending",
    noTransactionsFound = "No transactions found",

    // Settings
    settingsTitle = "Settings",
    configAndSettings = "Configuration & Preferences",
    deviceInfo = "Device Information",
    monitorSms = "SMS Monitoring",
    monitoringActive = "Active - listening for bank SMS",
    monitoringPaused = "Paused",
    approvalModeTitle = "Approval Mode",
    queueCount = "queued",
    serverConnections = "Server Connections",
    scanQr = "Scan QR",
    addManual = "Add Manual",
    noServers = "No servers configured",
    addServerToSync = "Add a server to sync payment data",
    scanQrToAddServer = "Scan QR Code to add server",
    supportedBanks = "Supported Banks",
    primaryBadge = "Primary",
    lastSyncLabel = "Last sync",
    neverSynced = "Never synced",
    successStatus = "Success",
    failedStatus = "Failed",
    removeButton = "Remove",
    removeServerTitle = "Remove Server",
    removeServerMessage = "Remove from connections? Server data will not be deleted.",
    cancelButton = "Cancel",
    addServerTitle = "Add Server",
    addServerSubtitle = "Connect to Laravel to sync payment data",
    serverNameLabel = "Server Name",
    serverNamePlaceholder = "e.g. Thaiprompt Main",
    serverUrlLabel = "Server URL",
    secretKeyLabel = "Secret Key (encryption)",
    setAsDefault = "Set as default server",
    saveButton = "Save",
    versionInfo = "SMS Payment Checker v${com.thaiprompt.smschecker.BuildConfig.VERSION_NAME}\n\u00A9 2025 Xman Studio",

    // Theme
    themeMode = "Theme",
    themeDark = "Dark",
    themeLight = "Light",
    themeSystem = "System",

    // Language
    languageTitle = "Language",
    languageThai = "ไทย",
    languageEnglish = "English",
    languageSystem = "System",

    // QR Scanner
    qrScannerTitle = "Scan QR Code",
    qrScannerSubtitle = "Add server from QR Code",
    cameraPermissionRequired = "Camera permission required",
    cameraPermissionDenied = "Camera permission needed to scan QR Code",
    cameraFailed = "Failed to open camera",
    pointCameraAtQr = "Point camera at server QR Code",
    howToGetQrCode = "How to get QR Code",
    qrInstructions = "1. Open your website Admin panel\n2. Go to SMS Checker settings\n3. Click 'Show QR Code' for device\n4. Scan the QR Code on screen",
    allowCamera = "Allow Camera",
    qrDefaultServerName = "Server from QR",

    // SMS History
    smsMatcherTitle = "SMS History",
    smsMatcherSubtitle = "Detected bank messages",
    customBankMapping = "Bank SMS History",
    customBankMappingDesc = "View detected bank messages",
    totalDetected = "Detected",
    totalSynced = "Synced",
    allTypes = "All",
    creditOnly = "Credit",
    debitOnly = "Debit",
    noTransactionsYet = "No messages detected yet",
    syncedLabel = "Synced",
    pendingLabel = "Pending",
    addRule = "Add Rule",
    noRules = "No rules yet",
    noRulesDescription = "Select SMS below to assign a bank",
    recentSmsTitle = "Recent SMS",
    assignBank = "Assign Bank",
    senderAddressLabel = "Sender",
    sampleMessageLabel = "Sample Message",
    selectBankLabel = "Select Bank",
    activeRules = "Active Rules",
    deleteRule = "Delete Rule",
    deleteRuleConfirm = "Delete this rule?",
    tapToAssign = "Tap to assign bank",

    // Bank Names
    bankKbank = "Kasikorn Bank",
    bankScb = "Siam Commercial Bank",
    bankKtb = "Krungthai Bank",
    bankBbl = "Bangkok Bank",
    bankGsb = "Government Savings Bank",
    bankBay = "Bank of Ayudhya",
    bankTtb = "TMBThanachart Bank",
    bankPromptPay = "PromptPay",
    bankCimb = "CIMB Thai",
    bankKkp = "Kiatnakin Phatra Bank",
    bankLh = "Land and Houses Bank",
    bankTisco = "TISCO Bank",
    bankUob = "United Overseas Bank",
    bankIcbc = "ICBC (Thai)",
    bankBaac = "BAAC",

    // SMS Auto Scan
    autoScanTitle = "Auto Scan SMS",
    autoScanSubtitle = "Find bank messages from inbox",
    detectedBankSms = "Detected Bank SMS",
    unknownSender = "Unknown Sender",
    allOtherSms = "All SMS on Device",
    tapToAssignBank = "Tap to assign bank",
    autoDetected = "Auto Detected",
    customRule = "Custom Rule",
    matchedWithOrder = "Matched with order",
    noMatchingOrder = "No matching order",
    scanInbox = "Scan Inbox",
    scanInboxHint = "Tap the button above to scan SMS messages on device",
    scanning = "Scanning...",
    scanComplete = "Scan Complete",
    foundMessages = "messages found",

    // TTS
    ttsTitle = "Voice Alerts",
    ttsDescription = "Read transaction alerts with AI voice",
    ttsEnabled = "Voice alerts on",
    ttsDisabled = "Voice alerts off",
    ttsCreditAnnounce = "Income from %s, amount %s baht",
    ttsDebitAnnounce = "Expense from %s, amount %s baht",
    ttsOrderMatch = "Matched with order %s",
    ttsLanguageLabel = "Speech Language",
    ttsLangAuto = "Follow App",
    ttsLangThai = "Thai",
    ttsLangEnglish = "English",
    ttsSpeakContentLabel = "Speech Content",
    ttsSpeakTypeLabel = "Transaction Type (Income/Expense)",
    ttsSpeakBankLabel = "Bank Name",
    ttsSpeakAmountLabel = "Amount",
    ttsSpeakOrderLabel = "Order Number",
    ttsSpeakCustomerLabel = "Bill Owner Name",
    ttsSpeakProductLabel = "Product Name",
    ttsPreviewButton = "Preview",

    // Notification Listening
    notificationListeningTitle = "Bank Notification Listener",
    notificationListeningActive = "Active - listening for bank app notifications",
    notificationListeningPaused = "Disabled",
    notificationAccessRequired = "Enable Notification Access in system settings",
    notificationBadge = "Notification",

    // SMS Permission
    smsPermissionRequired = "Enable SMS permission in app settings",

    // SMS History Edit
    editTransaction = "Edit Transaction",
    editBank = "Bank",
    editType = "Type",
    editAmount = "Amount",
    editSave = "Save",
    editCancel = "Cancel",
    editRawMessage = "Original Message",
    editIncorrectData = "Correct incorrect data",
    navSmsHistory = "Messages",

    // Common
    backButton = "Back",
)

val LocalAppStrings = compositionLocalOf { ThaiStrings }
val LocalThemeMode = compositionLocalOf { ThemeMode.DARK }
val LocalLanguageMode = compositionLocalOf { LanguageMode.THAI }
